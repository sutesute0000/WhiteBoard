...
# 推奨エッジ種別
edge.label は、関係を正確に表せる場合は次のいずれかにしてください。
- 理由: AがBの理由である
- 結果: Aの結果としてBが起きる
- 比較・対立: AとBが比較・対立している
- 前提: AがBの前提条件である
...
- 範囲外: BがAの対象範囲から除外される
- 時系列: AのあとにBが発生
- 上記で正確に表せない関係だけ、2〜8文字程度の短い日本語ラベルを自由記述してよい。
- 自由記述ラベルは動詞句または関係名にする。長文説明は禁止。
  ...
  "ops": [
    {
      ...
      "edges": [
        { "from": "A", "to": "B", "label": "理由|結果|比較・対立|前提|具体化|例|リスク|提案|結論|範囲|範囲外|時系列|短い自由記述" }
      ]
    }
  ],
  "rationale": "<図の選択理由を短く>"
